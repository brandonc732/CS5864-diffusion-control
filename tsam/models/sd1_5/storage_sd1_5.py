...
from tsam.models.processors import AttnProcessorX_2
...
class AttnFetchSDX_3():
    ...
    def get_unet_cross_attn_maps(self, unet):
        import numpy as np
        """
        Creates an array of cross attention maps from unet model

        For each cross attention map, the layers are averaged over
        """
        ...
        for _, block in enumerate(unet.down_blocks):
            if block.__class__.__name__ == "CrossAttnDownBlock2D":
                ...
                for i in range(2):
                    ...
                    # data is a (HxW, 77) shape tensor
                    # reshape to square and add to block layers
                    s = int(np.sqrt(data.shape[0]))
                    block_layers.append(data.detach().reshape(s, s, 77))
                
                # add average of block layers to unet_attn_data
                block_layer_tensor = torch.stack(block_layers, dim=0).mean(dim=0)
                unet_attn_data.append(block_layer_tensor)
        ...
        for _, block in enumerate(unet.up_blocks):
            if block.__class__.__name__ == "CrossAttnUpBlock2D":
                ...
                for i in range(2):
                    ...
                    # reshape to square and add to block layers
                    s = int(np.sqrt(data.shape[0]))
                    block_layers.append(data.reshape(s, s, 77))
